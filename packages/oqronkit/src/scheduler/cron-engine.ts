...
export class SchedulerModule implements IOqronModule {
	...
	private async handleLeaderInit(): Promise<void> {
		...
		for (const record of knownSchedules) {
			...
			if (result.missed) {
				...
				// Fire once per missed date for "run-all", or just once for "run-once"
	...
	private async detectClusterStalls() {
		try {
			...
			for (const job of activeDbJobs) {
				...
				const def = this.schedules.find(
					(s) => s.name === job.scheduleId,
				);
		...
		try {
			...
			for (const record of due) {
				...
				if (record.paused) {
					...
					if (behavior === "reject") {
						this.logger.warn(
							"Cron fire rejected — instance is disabled",
							{
								name: def.name,
								behavior: "reject",
							},
					...
					if (heldJobs.length > maxHeld) {
						const toRemove = heldJobs.slice(
							0,
							heldJobs.length - maxHeld,
						);
	...
	private async fire(def: CronDefinition, tickTime: Date = new Date()): Promise<void> {
		...
		if (isOverlapSkip) {
			for (const job of this.activeJobs.values()) {
				if (job.lockKey === `${this.lockPrefix}:run:${def.name}`) {
					this.logger.debug("Skipping overlapping run", {
						name: def.name,
					});
					return;
				}
		...
		if (def.maxConcurrent) {
			...
			for (const job of this.activeJobs.values()) {
				if (
					job.lockKey.startsWith(`${this.lockPrefix}:run:${def.name}`)
				)
					activeCount++;
			}
		...
		const lockKey = isOverlapSkip
			...
			: `${this.lockPrefix}:run:${def.name}:${tickTime.getTime()}`;
		...
		entry.promise = Promise.resolve().then(async () => {
			...
			const ctx = new CronContext({
				...
				onLog: (level, msg) => {
					...
					(this.logger as any)[level]?.(`[Cron:${def.name}] ${msg}`, {
						runId,
					});
						...
						const timeoutPromise = new Promise<never>(
							(_, reject) => {
								timeoutHandle = setTimeout(() => {
									...
									reject(
										new Error(
											`Handler timed out after ${def.timeout}ms`,
										),
									);
								}, def.timeout);
							},
			...
			// M1 Fix: Removed duplicate nextRunAt update.
			// tick() now owns pointer advancement via the At-Least-Once wrapper.
			...
			const keepFailed =
				...
				this.config?.keepFailedJobHistory ??
				true;
