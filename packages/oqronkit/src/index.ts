...
async function discoverTriggers(
	...
	if (typeof config.triggers === "string") {
		...
		try {
			...
			logger.info(
				`Loaded ${count} trigger file(s) from ${config.triggers}`,
			);
		} catch {
			logger.warn(
	...
	for (const probe of TRIGGER_PROBE_PATHS) {
		...
		try {
			...
			const count = await scanDir(resolved);
			logger.info(
				`Auto-discovered ${count} trigger file(s) from ${probe}/`,
			);
			return;
		} catch {
			// Not found — try next
...
export const OqronKit = {
	...
	async init(opts?: { cwd?: string; config?: OqronConfig }): Promise<void> {
		...
		_logger.info(
			`Starting OqronKit in "${_config.environment}" environment`,
		);
		...
		const cronConf = getModuleConfig<CronModuleDef>(
			_config.modules,
			"cron",
		);
		if (cronConf) {
		...
		const queueConf = getModuleConfig<QueueModuleDef>(
			_config.modules,
			"queue",
		);
		if (queueConf) {
		...
		if (webhookConf) {
			const { WebhookEngine } =
				await import("./webhook/webhook-engine.js");
		...
		if (ratelimitConf) {
			const { RateLimitModule } =
				await import("./ratelimit/ratelimit-module.js");
			const rlModule = new RateLimitModule(
				_config,
				_logger!,
				ratelimitConf,
			);
