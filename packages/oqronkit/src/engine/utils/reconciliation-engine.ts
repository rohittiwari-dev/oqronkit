...
export class ReconciliationEngine {
  ...
  async runScan(): Promise<number> {
    ...
    const lockTtlMs = Math.max((this.config.intervalMs ?? 120_000) * 2, 60_000);
    ...
    try {
      ...
      // Release leader lock
      await this.lock
        .release(this.leaderLockKey, this.leaderLockOwnerId)
        .catch(() => {});
    }
  }
  ...
  private async reconcileDelayed(
    ...
    for (const job of filtered) {
      ...
      if (job.runAt) {
        ...
        shouldBeReadyAt = new Date(job.createdAt).getTime() + job.opts.delay;
