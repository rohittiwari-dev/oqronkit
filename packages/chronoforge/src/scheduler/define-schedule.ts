 ...
 * Automatically registers it with ChronoForge for boot-time loading.
 */
  ...
  return {
    ...
    trigger: async (opts?: EnqueueOptions<TPayload>) => {
      ...
      if (!engineRef.current) {
        throw new Error(
          `[ChronoForge] Cannot trigger "${options.name}" — ScheduleEngine is not running.`,
        );
      }
    ...
    schedule: async (opts?: EnqueueOptions<TPayload>) => {
      if (!engineRef.current) {
        throw new Error(
          `[ChronoForge] Cannot schedule "${options.name}" — ScheduleEngine is not running.`,
        );
      }
