...
const ITEMS = [
  {
    ...
    name: "Task Queue",
    description:
      "Unified, simple queue for monolithic setups where publisher and consumer live together. FIFO, LIFO, priority ordering, delayed jobs, and retry policies — all built in.",
    icon: Box,
    badge: "core",
    ...
    cta: "Read the docs",
    tags: ["FIFO", "Priority", "Delayed", "Retry"],
  },
  {
    key: "worker",
    label: "Distributed",
    ...
    cta: "View worker",
    tags: ["Polling", "Stall Detection", "Heartbeat", "Retry"],
  },
  {
    key: "scheduler",
    label: "Time-based",
    name: "Scheduler & Cron",
    description:
      "Cron expressions, RRule, one-shot runAt, repeating runAfter, and semantic recurring schedules. Sharded leader election prevents thundering herds.",
    icon: Clock,
    badge: "advanced",
    ...
    href: "/docs/scheduler",
    cta: "View scheduler",
    tags: ["Cron", "RRule", "Timezone", "Sharded"],
  },
  {
    key: "ratelimit",
    label: "Protection",
    name: "Rate Limiter",
    description:
      "Distributed sliding-window, fixed-window, and token-bucket algorithms. Multi-tier limits with automatic banning for abuse prevention. Atomic operations via Lua scripts.",
    icon: Shield,
    badge: "security",
    ...
    cta: "View limiter",
    tags: ["Multi-tier", "Auto-ban", "Token Bucket", "Atomic"],
  },
  {
    key: "webhook",
    label: "HTTP Dispatch",
    name: "Webhook Engine",
    description:
      "Reliable outbound webhook delivery with exponential backoff, dead letter queues, circuit breakers, HMAC-SHA256 signing, and dynamic endpoint resolution.",
    icon: Zap,
    badge: "reliable",
    ...
    href: "/docs/webhook",
    cta: "View webhooks",
    tags: ["Circuit Breaker", "DLQ", "HMAC Signing", "Retry"],
  },
  {
    key: "docs",
    label: "Documentation",
    name: "Full Reference",
    description:
      "Comprehensive guides, API reference, integration examples, and architecture deep-dives — everything you need from first install to production deployment.",
    icon: BookOpen,
    badge: "open",
    ...
    href: "/docs",
    cta: "Browse docs",
    tags: ["Quick Start", "API Reference", "Adapters", "Crash Safety"],
  },
] as const;
          ...
          {ITEMS.map((item) => {
            ...
            return (
              <motion.div key={item.key} variants={cardVariants}>
                <Link
                  href={item.href}
                  className={`group relative flex flex-col h-full rounded-2xl border bg-fd-card/60 backdrop-blur-sm p-7 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl ${item.border}`}
                >
                  <div
