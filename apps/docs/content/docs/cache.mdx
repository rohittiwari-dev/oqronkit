...
> [!TIP]
> **Working example →** See a complete Cache implementation with SWR, stampede protection, negative caching, prewarming, and lifecycle hooks:
> [apps/backend/src/services/caches.ts](https://github.com/rohittiwari-dev/oqronkit/blob/main/apps/backend/src/services/caches.ts)
...
|-------|-------|-----------|
| **Single-Flight** | Per-process | In-memory dedup — only 1 fetch per key per Node.js process |
| **Cluster Lock** | Per-cluster | Distributed lock via Lock adapter — only 1 fetch per key across ALL servers |
